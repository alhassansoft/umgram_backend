...
(async () => {
  try {
    ...
    const pool = new Pool({ connectionString: dbUrl, application_name: 'seed-user4' });

    const username = 'user4';
    const email = 'user4@local';
    ...
    if (exist[0]) {
      console.log('user4 already exists:', exist[0].id);
      await pool.end();
      process.exit(0);
    }
    ...
    `;
  const vals = [email, username, 'User Four', hash, 'member', 'active'];
    const { rows } = await pool.query(insertSql, vals);
    console.log('user4 created:', rows[0]);
    await pool.end();
  } catch (e) {
    console.error('Seed user4 failed:', e);
    process.exit(1);
  }
})();
