...
(async () => {
  try {
    ...
    const pool = new Pool({ connectionString: dbUrl, application_name: 'seed-admin' });
    ...
    if (exist[0]) {
      console.log('Admin already exists:', exist[0].id);
      await pool.end();
      process.exit(0);
    }
    ...
    `;
    const vals = [email, username, 'Admin', hash, 'admin', 'active'];
