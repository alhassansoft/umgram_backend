...
(async () => {
  try {
    ...
    const pool = new Pool({ connectionString: dbUrl, application_name: 'seed-user6' });

    const username = 'user6';
    const email = 'user6@local';
    const password = 'user6';

    const { rows: exist } = await pool.query('SELECT id FROM users WHERE username=$1 OR email=$2 LIMIT 1', [username, email]);
    if (exist[0]) {
      console.log('user6 already exists:', exist[0].id);
      await pool.end();
      process.exit(0);
    }
    ...
    `;
  const vals = [email, username, 'User Six', hash, 'member', 'active'];
    const { rows } = await pool.query(insertSql, vals);
    console.log('user6 created:', rows[0]);
    await pool.end();
  } catch (e) {
    console.error('Seed user6 failed:', e);
    process.exit(1);
  }
})();
