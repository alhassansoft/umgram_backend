...
(async () => {
  try {
    ...
    const pool = new Pool({ connectionString: dbUrl, application_name: 'seed-user3' });

    const username = 'user3';
    const email = 'user3@local';
    ...
    if (exist[0]) {
      console.log('user3 already exists:', exist[0].id);
      await pool.end();
      process.exit(0);
    }
    ...
    `;
  const vals = [email, username, 'User Three', hash, 'member', 'active'];
    const { rows } = await pool.query(insertSql, vals);
    console.log('user3 created:', rows[0]);
    await pool.end();
  } catch (e) {
    console.error('Seed user3 failed:', e);
    process.exit(1);
  }
})();
