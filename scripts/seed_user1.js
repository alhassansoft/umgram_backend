...
(async () => {
  try {
    ...
    const pool = new Pool({ connectionString: dbUrl, application_name: 'seed-user1' });

    const username = 'user1';
    const email = 'user1@local';
    const password = 'user1';

    const { rows: exist } = await pool.query('SELECT id FROM users WHERE username=$1 OR email=$2 LIMIT 1', [username, email]);
    if (exist[0]) {
      console.log('user1 already exists:', exist[0].id);
      await pool.end();
      process.exit(0);
    }
    ...
    `;
  const vals = [email, username, 'User One', hash, 'member', 'active'];
    const { rows } = await pool.query(insertSql, vals);
    console.log('user1 created:', rows[0]);
    await pool.end();
  } catch (e) {
    console.error('Seed user1 failed:', e);
    process.exit(1);
  }
})();
