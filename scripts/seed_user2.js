...
(async () => {
  try {
    ...
    const pool = new Pool({ connectionString: dbUrl, application_name: 'seed-user2' });

    const username = 'user2';
    const email = 'user2@local';
    ...
    if (exist[0]) {
      console.log('user2 already exists:', exist[0].id);
      await pool.end();
      process.exit(0);
    }
    ...
    `;
  const vals = [email, username, 'User Two', hash, 'member', 'active'];
    const { rows } = await pool.query(insertSql, vals);
    console.log('user2 created:', rows[0]);
    await pool.end();
  } catch (e) {
    console.error('Seed user2 failed:', e);
    process.exit(1);
  }
})();
